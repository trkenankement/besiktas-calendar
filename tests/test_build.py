...
from besiktas_calendar import build, cli
...
def test_successful_run_writes_every_output_and_splits_the_feeds_by_sport(tmp_path, monkeypatch):
    ...
    assert build.run(tmp_path, providers=working_providers(), session=object(), now=NOW) == 0
...
def test_output_folder_is_created_when_missing(tmp_path):
    ...
    assert build.run(out, providers=working_providers(), session=object(), now=NOW) == 0
    assert (out / "besiktas-all.ics").exists()


def test_a_second_run_with_unchanged_data_rewrites_nothing_but_the_check_time(tmp_path):
    build.run(tmp_path, providers=working_providers(), session=object(), now=NOW)
    before = {p.name: p.read_bytes() for p in tmp_path.iterdir() if p.name != "last_check.txt"}

    later = NOW + timedelta(minutes=30)
    assert build.run(tmp_path, providers=working_providers(), session=object(), now=later) == 0
...
def test_unexpected_exceptions_are_reported_as_source_failures(tmp_path):
    outcomes = build.collect([provider("x", error=KeyError("alan"))], session=object(), today=date(2026, 9, 20))
...
def test_the_same_match_from_two_sources_is_published_once(tmp_path):
    ...
    build.run(tmp_path, providers=providers, session=object(), now=NOW)
    assert len(events_in(tmp_path / "besiktas-football.ics")) == 6
...
def test_github_actions_gets_annotations_and_a_step_summary(tmp_path, capsys, monkeypatch):
    ...
    assert build.run(tmp_path / "docs", providers=providers, session=object(), now=NOW) == 1
...
def test_cli_forwards_the_output_folder(monkeypatch):
    received = []
    monkeypatch.setattr(cli, "run", lambda out: received.append(out) or 0)

    assert cli.main(["--out", "cikti"]) == 0
    assert cli.main([]) == 0
    assert received == [Path("cikti"), Path("docs")]
